(() => {
  ...
  const tokenPriceText = (value) => {
    ...
    if (number !== 0 && Math.abs(number) < 1e-7) return number.toExponential(6);
  ...
  function renderStrategyModal() {
    ...
    const textFor = (key, value) => {
      const names = {
        ...
        pricing_mode: '定价模式：BSC 链上只读报价', executable_quote: value ? '可执行只读报价：是（不广播）' : '可执行报价：否', require_readonly_buy_quote: '必须取得非零链上买入 Quote', require_readonly_sell_quote: '必须取得非零链上即时卖出 Quote', binance_indicative_reference_only: 'Binance 指示价仅作 Dashboard 对照，不参与 PnL', observation_delay_sec: `观察期 ${observation}`, observation_price_rise_required: value ? `${observation}观察后价格必须高于首次发现价格` : `${observation}观察后价格不得低于首次发现价格`, require_holders_non_decreasing_after_observation: value ? `${observation}观察后持币地址数不得低于首次发现值` : '观察后持币地址数不设下降门槛', optional_unavailable_fields: `不可用字段（仅记录、不阻断）：${Array.isArray(value) ? value.join('、') : value}`, position_size_bnb: `单笔虚拟仓位 ${value} BNB`, initial_virtual_balance_bnb: `初始虚拟余额 ${value} BNB`, daily_full_loss_bnb: `每日完整亏损上限 ${value} BNB`,
      };
      if (key === 'rules') {
        const ruleLabels = {
