...
class Phase4Tests(unittest.TestCase):
    ...
    def test_dashboard_control_and_config_service_are_whitelisted(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(bsc_display["display_name"], "BSC Binance 指示价 Paper/Shadow 策略")
            self.assertEqual(bsc_display["pricing"]["pricing_mode"], "binance_indicative")
            self.assertFalse(bsc_display["pricing"]["executable_quote"])
            self.assertIn("binance_current_price_required", bsc_display["strategy_config"]["entry"])
